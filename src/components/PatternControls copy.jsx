// src/components/PatternControls.jsx — UPDATED WITH PATTERN TYPE DROPDOWN
import React from 'react'
import useStore from '../store/useStore'
import { detectStreaks } from '../lib/detectStreaks'
...
export default function PatternControls() {
  ...
  React.useEffect(() => {
    ...
    setStreaks(streaks)
  }, [candles, minStreak, direction, minMovePercent])

  const streaks = useStore.getState().streaks || []
  const total = streaks.length
  const reversals = streaks.filter(s => s.isReversal).length
  const continuations = streaks.filter(s => s.isContinuation).length
  const strong = streaks.filter(s => s.isStrong).length
