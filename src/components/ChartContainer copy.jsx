// src/components/ChartContainer.jsx — UPDATED WITH TYPE FILTER
...
export default function ChartContainer() {
  ...
  const { candles, streaks, patternTypeFilter } = useStore()
  ...
  useEffect(() => {
    ...
    if (markersPrimitiveRef.current) {
      const markers = streaks
        .filter(s => s.brokeExtreme && s.nextCandle)
        .filter(s => {
  ...
  }, [candles, streaks, patternTypeFilter])
  ...
  return (
    <div className="mt-8 bg-slate-900 rounded-2xl p-6 shadow-2xl border border-gray-800">
      ...
      <div className="mt-6 flex justify-center gap-8 text-sm text-gray-300">
